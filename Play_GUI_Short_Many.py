...
class App(QWidget):
    ...
    @pyqtSlot()
    def startClick(self):
        global disboard, cnt, nowply, isGameing, searchDeep
        disboard, cnt = othello.initializeGame()
        nowply = 1
    ...
    def searchValueDown(self, nowBoard, color, mycolor, cnt, repeat):
        ...
        # 움직일 수 있는 곳이 한곳보다 많다
            ...
            if len(canMovF) == 0:
                ...
                if len(canMovF) == 0:
                    if semimode:
                        sorting = c.deepcopy(canMovA)
                        for i in range(len(sorting)):
                            sorting[i] = abs(1 - canMovA[i])
                        sorting.sort()
                        if len(sorting) > minMode:
                            sorting = sorting[:minMode]

                        for z in sorting:
                            for i in range(len(canMovA)):
                                if abs(1 - canMovA[i]) == z:
                                    canMovF.append(canMove[i])
                                    break
                    else:
                        canMovF = list()
    ...
    def aiMove(self):
        global cnt, disboard, nowply, ply, isGameing, searchDeep
                ...
                if len(othello.isEnableToMove(disboard, nowply)) == 0:
                    ...
                    if len(sameM) == 1:
                        ...
                        self.aiWinRate.setText(
                            " 인공지능 예상 승률: " + str("{0:.2f}".format(
                                mcts[sameNum[sameRandom]][0] / mcts[sameNum[sameRandom]][1] * 100)) + "%")
                        cnt += 1
                        ...
                        insameBig = mcts[sameNum[0]][1]
                        for i in range(1, len(sameNum)):
                            if insameBig < mcts[sameNum[i]][1]:
                                insameBig = mcts[sameNum[i]][1]
                        insameBigM = list()
                        ...
                        self.aiWinRate.setText(
                            " 인공지능 예상 승률: " + str("{0:.2f}".format(
                                mcts[insameBigM[sameRandom]][0] / mcts[insameBigM[sameRandom]][1] * 100)) + "%")
                        cnt += 1

                        othello.move(canMov[insameBigM[sameRandom]], disboard, nowply)
                        nowply = -1 * nowply
                        self.resetBoard(disboard, nowply, canMov[insameBigM[sameRandom]])
